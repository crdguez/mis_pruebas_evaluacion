...
\newcommand{\examnum}{Global de Análisis}
\newcommand{\tipo}{B}
...
% \begin{parts}
%     \part[1] $\int\left(x^6-3+\frac{2}{x}\right)dx$
%     % \part[1] $\int\left(x^5-2+\frac{4}{x^2}\right)dx$
%     % \part[1] $\int x^2\sqrt{x} \ dx$
%     \part[1] $\int x^3\sqrt{x} \ dx$
%     % \part[1] $\int x\cdot  e^{x^2+1} \ dx$
%     \part[1] $\int\left(\sqrt{x}-\sqrt{2}\right)^2dx$
%     % \part[1] $\int \dfrac{2x}{5x^2+9}\ dx$
%     \part[1] $\int \dfrac{3x}{6x^2-7}\ dx$
% \end{parts}


%  2 examen

\question  Dada la función $f(x)=\dfrac{x^2-4x+12}{x-1}$:
\begin{parts}
    \part[1] Calcula el dominio
    % \part[2] Calcula la asíntotas 
    % \begin{solution}
    %     $x=1$ y $y=x-3$
    % \end{solution}
    \part[2] Máximos y mínimos relativos de $f$
    \begin{solution}
        m $(4,4)$ y M $(-2,-8)$
    \end{solution}
    \part[1]  Calcular las rectas tangentes a $f$ que sean paralelas al eje $OX$ 
\end{parts}

% \question Dada $f(x)=\dfrac{mx-1}{x^2}$:
% % \question Dada $f(x)=\dfrac{mx^3-1}{x^2}$:
% \begin{parts}
%     \part[2] Determine el valor del parámetro $m$ para que la función tenga un extremo relativo en $x=-1$. Razone si se trata de un máximo o un mínimo relativo
% \end{parts}
...
\begin{parts}
    \part[2] Determinar cuándo la empresa tiene ganancias y cuándo pérdidas.
    % \part[2] Determinar si $B(t)$ tiene máximos, mínimos relativos y puntos de inflexión en su dominio de definición.
    % \begin{solution}
    %     No tiene
    % \end{solution}
    \part[2] Determinar cuándo los beneficios crecen
    \part[2]  ¿Están los beneficios limitados?. Razonar la respuesta indicando los beneficios.
\end{parts}
...
\begin{parts}
    ...
    % \part[1] $\int x\cdot  e^{x^2+1} \ dx$
    \part[1] $\int\left(\sqrt{2x}-\sqrt{3}\right)^2dx$
    \part[1] $\int \dfrac{2x}{5x^2+9}\ dx$
    % \part[1] $\int \dfrac{3x}{6x^2-7}\ dx$
\end{parts}
