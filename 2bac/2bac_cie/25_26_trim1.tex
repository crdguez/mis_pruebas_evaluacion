...
$ Por tanto $f'(x)=0 \iff x=-1$ \\ 
Como $\lim_{x \to \infty}\left(\left(x + 2\right) e^{- x}\right)
=0 \ \land \ \lim_{x \to -\infty}\left(\left(x + 2\right) e^{- x}\right)
=-\infty \ \land \ dom(f)=\mathbb{R} \to $ en $x=-1$ hay un mínimo relativo y absoluto $\to (-1,e)$, el mínimo absoluto es $-\infty$
\end{solution}

\part[50] Determina la concavidad y convexidad y los puntos de inflexión de la función \( f \).
\begin{solution}
    $f''(x)=x e^{- x}$, por tanto $f''(x)=0 \iff x=0$ \\
    $f''(x)<0 \ si \in (-\infty,0) \land f''(x)>0 \ si \in (0,\infty)$, por tanto pasa de cóncava a convexa y hay un punto de inflexión en $x=0 \to (0,2)$ 
\end{solution}
...
\begin{solution}
    ...
    Asíntota oblicua: $\lim_{x \to -\infty}\left(\frac{\left(x + 2\right) e^{- x}}{x}\right)=\infty \to \nexists$
\end{solution}

\part[100] Halla la recta tangente a la gráfica de la función en $x=-1$
\begin{solution}
    $f'(-1)=0 \to y-e=0\cdot(x+1) \to y=e$
\end{solution}
\end{parts}
...
\begin{solution}
    $f(x)=\frac{x \sqrt{64 - x^{2}}}{2}$, por tanto $f'(x)=\frac{32 - x^{2}}{\sqrt{64 - x^{2}}}$, luego $f'(x)=0 \iff x=4\sqrt{2}$. Se comprueba que ahí hay máximo absoluto
\end{solution}
...
inflexión
\begin{solution}
    $f'(x)=a + 15 x^{4} + 30 x^{2} \ \land \ f''(x)=60 x^{3} + 60 x$, luego
    $f''(x)=0 \iff x=0, x=\pm \sqrt-1$. iComo $f'''(0)=60\neq0$, En $x=0$ está el único punto de inflexión ya que $f'''(0)=60\neq0$
\end{solution}
    \part[100] Hallar a y b para que la tangente a la gráfica de dicha función en el punto de
inflexión sea la recta $y = x + 2$
\begin{solution}
    $f'(0)=1 \land (0,2) \in f(x)$ luego $a=1 \ \land \ b=2 $
\end{solution}
\end{parts}
...
$$\lim_{x \to 0}\left(\frac{\ln{\left(x + e^{x} \right)}}{x}\right)$$
    \begin{solution}
        $\lim_{x \to 0}\left(\frac{\ln{\left(x + e^{x} \right)}}{x}\right)=\lim_{x \to 0}\frac{\frac{e^x+1}{x+e^x}}{1}=2$
    \end{solution}
